	...
	function _initialize() {
		...
		if($ucenter_syn){
			...
			if(empty($session_user)){
				if(!empty($_COOKIE['WinLangCMS_auth'])  && $_COOKIE['WinLangCMS_auth']!="logout"){
					$WinLangCMS_auth=sp_authcode($_COOKIE['WinLangCMS_auth'],"DECODE");
					$WinLangCMS_auth=explode("\t", $WinLangCMS_auth);
					$auth_username=$WinLangCMS_auth[1];
					$users_model=M('Users');
