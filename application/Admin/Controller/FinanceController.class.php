...
class FinanceController extends AdminbaseController{
    ...
    public function add_post(){
        if (IS_POST) {
            ...
            $result=$this->finance_model->add($contracts);
            if ($result) {
                $finance['type']=1;
                $finance['source']=1;
    ...
    public function edit_post(){
        if (IS_POST) {
            ...
            $info=$this->finance_model ->where(array("id"=>$contracts['id']))->find();
            ...
            if(!empty($info)){
                ...
                $result=$this->finance_model ->save($contracts);
                if ($result!==false) {
                    $this->success("保存成功！");
                } else {
                    $this->error("保存失败！");
    ...
    public function export(){
        ...
        FinanceModel::exportOrderList($list);
        exit;
    }
...
//        }else{// 上传成功
//            $import=FinanceModel::import($filename, $exts);
